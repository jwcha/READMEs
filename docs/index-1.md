```#README #first #meta #2017#12#01```
# THE READMEs
-----

+ File extension ".rme" will be used for README files.
+ The first line of all README files will contain whitespace-delimited tags. Tags may be nested by using octothorpes. e.g. #topic#sub-topic

-----

## some general rules

1. My README files (with extension .rme) are to be simple text files encoded in UTF-8 format (with UNIX newlines).
2. All lines in my README files will contain at most 80 charaters.
3. Consult the following table for special notation:

| effect | my notation | description |
| :---- | :------: | ----: |
| underline | ```_phrase_``` | I surround the word/phrase with underscores. |
| bold | ```*phrase*``` | I surround the word with asterisks. |
| exponents | ```x**y``` | 'x' raised to the power of 'y'. |


------

## some real boring history
...
> designed by people exceeded that figure, unles there was a guarantee of
...
> use a command-line based mailing client with a custom set-up and not a
> user-friendly solution like GMail or other free online service.)
